...
"""
Extract and synchronize messages from a ROS bag file recorded on a
robotic ultrasound rig (for example, an ultrasound imager plus the
robot's end-effector pose stream).
...
ROS versions and non-ROS rigs:
    ...
    unchanged. Non-ROS rigs, such as robot SDK logs (for example dVRK or
    KUKA FRI state streams) or frame-grabber SDK captures, can enter the
    pipeline at the temp_cali.py stage with any pair of timestamped arrays.

This tool is useful for robot learning applications where sensor streams
(ultrasound images, end-effector poses, robot state) arrive on different
topics at different rates and need to be temporally aligned before conversion
to the LeRobot format. It provides a way to:
...
2. Apply optional per-topic time offsets to compensate for sensor delays
   (for example, frame-grabber latency on the ultrasound image path).
...
- The demonstration at the bottom synchronizes an ultrasound image stream
  with the robot's end-effector pose stream and writes the result to HDF5.

Typical use case:
- You recorded a robotic ultrasound demonstration (phantom, ex-vivo, or
  clinical) with an ultrasound imager and the robot's end-effector pose
  stream.
- The two streams publish at different frequencies, and the image path adds
  latency (frame grabber, capture card), so raw timestamps are misaligned.
- You want time-synchronized tuples {ultrasound image, end-effector pose} to
  build training episodes. Estimate the per-topic offsets first with
  temp_cali.py, then apply them here.
...
if __name__ == '__main__':
    ...
    topics:     /robot/ee_pose           1858 msgs    : geometry_msgs/PoseStamped
                /ultrasound/image_raw     557 msgs    : sensor_msgs/Image
    '''
    image_topic = "/ultrasound/image_raw"
    pose_topic = "/robot/ee_pose"

    # Per-topic time offsets in seconds, added to each recorded timestamp.
    # The ultrasound images pass through a frame grabber before they reach
    # ROS, so their stamps lag the true acquisition time. Subtracting the
    # measured latency (here 0.08 s, estimated with temp_cali.py) realigns
    # the image stream with the end-effector pose stream.
    time_offsets = {
        image_topic: -0.08,
        pose_topic: 0.0,
    }
    ...
    for synced_msgs in extractor.synchronize_messages():
        ...
        for (topic, _), msg in zip(topics_list, synced_msgs):
            if topic == image_topic:
                ...
                # (joint states, force-torque readings, probe acquisition
                # parameters, teleoperation commands, ...).
                pass
